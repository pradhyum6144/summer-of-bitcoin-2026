...
**How it is detected/computed:**
Each output value is checked: `value % 100_000 == 0` and `value > 0`. Detected if any output satisfies this condition.
...
**How it is detected/computed:**
Compare raw scriptPubKey bytes from undo data (input prevouts) against all output scriptPubKeys. If any match, the address is being reused within the same transaction.
...
- The transaction has at most 2 outputs and at most 3 inputs.
- These constraints distinguish self-transfers from consolidations (which have many inputs).

**Confidence model:**
Medium confidence. Matching script types is suggestive but not conclusive.

**Limitations:**
- Payments to the same address type (e.g., P2WPKH → P2WPKH) will be incorrectly flagged.
- Wallet migrations between address types won't be detected.
...
- **Fee calculation**: `fee = sum(input_values) - sum(output_values)`, using prevout values from undo data. Fee rate = `fee / vsize` where `vsize = weight / 4`.
...
- **Heuristic thresholds**: CoinJoin detection requires 3+ equal outputs (not 2) to reduce false positives. Consolidation requires 5+ inputs. These are conservative thresholds.
