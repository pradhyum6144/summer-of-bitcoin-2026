...
#
# Source this file from grader scripts:
...
# run_cli <args...>
#
# Runs ./cli.sh with a 180-second timeout (increased for large block files).
...
run_cli() {
  ...
  set +e
  run_with_timeout 180 ./cli.sh "$@" >"$tmpout" 2>"$tmperr"
  CLI_EXIT=$?
  set -e
  ...
  if [[ $CLI_EXIT -eq 124 ]]; then
    CLI_STDERR="TIMEOUT: cli.sh did not finish within 180 seconds"
  fi
}
